...
## Other Readers
To keep external dependencies to a minimum, only the base R `read.csv()` and `readLines()` are directly supported.  If you wish to use other readers like `fread()` from **data.table** [[@datatable]] or `read_csv()` from **readr** [[@readr]], you can pass one of these as the argument `reader` to the \code{sample_csv()} function.  However, this support has not been extensively tested, and enforces some strict rules about arguments.  See the help for details.

Note that if the downsampling was sufficiently aggressive, you will not notice any performance improvement using these better readers over `read.csv()`.
...
The package should perform very well, provided the total number of lines sampled is fairly small.  For a small (in absolute, not relative terms) number of lines, the total time spent should be dominated by R reading in the downsampled file, not the downsampling itself.
...
Obviously the **lineSampler** strategy is valuable only for aggressive downsampling, and not resampling.  Finally, we note that these benchmarks were performed on a machine with an ssd.  We would expect the overall timings to be larger for a platter disk, but the relative performance should be fairly similar.
...
Here we describe the implementation details for the file sampler functions.  The other package export, `wc()`, has been aggressively optimized; but its implementation is not very interesting, so we do not belabor it here.
